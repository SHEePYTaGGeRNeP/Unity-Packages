...
namespace Assets.ObjectPool.Scripts
{
    ...
    public sealed class ObjectPool : MonoBehaviour
    {
        ...
        [SerializeField]
        private StartupPoolMode _startupPoolMode = StartupPoolMode.CallManually;
        [SerializeField]
        ...
        {
            _instance = this;
            if (this._startupPoolMode == StartupPoolMode.Awake)
                CreateStartupPools();
        }
        ...
        private void Start()
        {
            if (this._startupPoolMode == StartupPoolMode.Start)
                CreateStartupPools();
        }
        ...
        public static void CreateStartupPools()
        {
            ...
            StartupPool[] pools = Instance._startupPools;
            if (pools == null || pools.Length <= 0) return;
            foreach (StartupPool pool in pools)
        ...
        {
            foreach (KeyValuePair<GameObject, GameObject> item in Instance._spawnedObjects.Where(item => item.Value == prefab))
                _tempList.Add(item.Key);
            foreach (GameObject go in _tempList)
                Recycle(go);
            _tempList.Clear();
        }
        ...
        public static List<T> GetPooled<T>(T prefab, List<T> list, bool appendList) where T : Component
        {
            ...
            if (!appendList)
                list.Clear();
            List<GameObject> pooled;
            if (!Instance._pooledObjects.TryGetValue(prefab.gameObject, out pooled)) return list;
        ...
        {
            List<GameObject> pooled;
            if (!Instance._pooledObjects.TryGetValue(prefab, out pooled)) return;
